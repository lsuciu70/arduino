...
void printBuff(uint8_t* buff, uint16_t len)
{
  for (uint16_t i = 0; i < len; ++i)
  {
    ...
    {
      Serial.print(" 0x");
      Serial.print((unsigned short) c, HEX);
      Serial.print(" ");
    }
  }
  if (len)
...
MqttClient::MqttClient(IPAddress addr, uint16_t port, MQTT_CALLBACK_SIGNATURE,
    Client& client)
{
  this->_state = MQTT_DISCONNECTED;
  setServer(addr, port);
...
MqttClient::MqttClient(uint8_t *ip, uint16_t port, MQTT_CALLBACK_SIGNATURE,
    Client& client)
{
  this->_state = MQTT_DISCONNECTED;
  setServer(ip, port);
...
boolean MqttClient::connect(const char *id, const char* willTopic,
...
boolean MqttClient::publish(const char* topic, const uint8_t* payload,
...
boolean MqttClient::publish_P(const char* topic, const uint8_t* payload,
    unsigned int plength, boolean retained)
{
  uint8_t llen = 0;
  uint8_t digit;
...
boolean MqttClient::write(uint8_t header, uint8_t* buf, uint16_t length)
{
  ...
  printBuff(buf + (4 - llen), length + 1 + llen);
