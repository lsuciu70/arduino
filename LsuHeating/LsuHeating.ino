...
void updatePxRunToday()
{
    for (byte i = 0; i < SENZOR_COUNT; ++i)
    {
        ...
        if ((programming[i] == P2_START_HOUR_1_INCREASE_05
                && delta_minute_p2 > 0) || (delta_minute_p2 > 2 * 60))
        {
        ...
        if ((programming[i] == P3_START_HOUR_2_INCREASE_05
                && delta_minute_p3 > 0) || (delta_minute_p3 > 2 * 60))
        {
    ...
    for (byte i = 0, j = offset; i < SENZOR_COUNT; ++i, ++j)
    {
        String p =
                String("<!-- room: ") + room_name[j] + " -->\n" + " <tr>\n"
                        ...
                        + "  <td align='center'>"
                        + ((1.0 * temperature[i]) / 100) + " &deg;C</td>\n"
                        + "  <td align='center'>"
                        + ((is_running[i]) ?
                                "<font color='red'>DA</font>" :
                                "<font color='blue'>NU</font>") + "</td>\n"
...
// Serial.println("Start sendHttpProgramming");
    ...
    String p = String("<!-- room: ") + room_name[(index + offset)] + " -->\n"
            + " <tr>\n" + "  <td rowspan=5>" + room_name[(index + offset)]
            + "</td>\n\n" + "<!-- programm: 0 -->\n" + "  <td align='left'>\n"
            ...
            + ((programming[index] == P1_RUN_HOURS_MAKE_TEMP) ?
                    "' checked" : "'") + ">"
            + printProgramming(index, P1_RUN_HOURS_MAKE_TEMP) + "</label><br>\n"
            + "   Start\n"
            ...
            + ((start_minute_p1[index] < 10) ? "0" : "")
            + start_minute_p1[index] + "'>\n" + "   Stop\n"
            + "   <input type='text' style='text-align:center;' name='"
            ...
            + ((start_minute_p2[index] < 10) ? "0" : "")
            + start_minute_p2[index] + "'>\n" + "   Urmatorul program: \n"
    ...
    p = String("") + "<!-- programm: 3 -->\n" + " <tr>\n"
            ...
            + ((start_minute_p3[index] < 10) ? "0" : "")
            + start_minute_p3[index] + "'>\n" + "   Urmatorul program:\n"
            ...
            + ((programming[index] == P4_START_NOW_INCREASE_05) ?
                    "' checked" : "'") + ">"
            + printProgramming(index, P4_START_NOW_INCREASE_05)
            + "</label><br>\n" + "   Urmatorul program:\n"
...
int savePostData_Programming(const String &data, int programm)
{
    ...
    if (name.equals(PROGRAMMING))
    {
        ...
        if (!valid || programm < 0 || programm >= P_AFTER_LAST)
        {
            ...
            writeLogger(
                    String("[") + T_LOC
                            + "] EROARE salvare programare - program necunoscut:'"
                            + value + "'");
            return -1;
        }
        if (programm != programming[index])
        {
            writeLogger(
                    String("[") + T_LOC + "] " + room_name[(index + offset)]
                            + " - programul s-a schimbat, program nou: "
                            + programming[index] + ", program vechi: "
                            + programm);
        }
        programming[index] = programm;
        return programm;
    }
    String bad_name = "", old_value = "",
            bad_room = room_name[(index + offset)];
...
//    Serial.println(String("ERROR programm ") + programm + " : " + data + " -> " + name + "[" + index + "] = " + value);
        writeLogger(
                String("[") + T_LOC
                        ...
                        + bad_name + ":'" + value + "' (apelat cu '" + data
                        + "') pentru " + bad_room
                        + "; ramane valoarea dinainte: " + old_value);
    }
...
void processPostData_Programming(const String &post_data)
{
    ...
    {
        programm = savePostData_Programming(post_data.substring(i, j),
                programm);
        i = j + 1;
    }
    if (i)
...
bool processPostData_Register(const String &post_data)
{
    ...
    if (ok)
    {
        writeLogger(
                String("[") + T_LOC + "] Inregistrare notificari temperatura: "
                        ...
                        + "; registri liberi: " + (MAX_REGISTER - last_register)
                        + " din: " + MAX_REGISTER);
        ++last_register;
    }
    else
    {
        writeLogger(
                String("[") + T_LOC
                        + "] EROARE - Inregistrare notificari temperatura: "
                        + post_data);
    }
    return ok;
}
...
bool processPostData_Unregister(const String &post_data)
{
    ...
    if (name.equals(REQUESTER_IP))
    {
        ++i;
        String value =
                (j > i) ? post_data.substring(i, j) : post_data.substring(i);
        bool found = false;
        byte addr[4];
        ...
        for (int k = 0; k < last_register; ++k)
        {
            ...
            if (found)
            {
                ...
                writeLogger(
                        String("[") + T_LOC
                                + "] Deinregistrare notificari temperatura: "
                                + value);
                writeLogger(
                        String("[") + T_LOC
                                + "] Deinregistrare notificari temperatura; registri liberi: "
                                + (MAX_REGISTER - last_register) + " din: "
                                + MAX_REGISTER);
            }
        }
        return true;
                ...
                if (c == '\n' && currentLineIsBlank)
                {
                    ...
                    if (req_str.indexOf("POST") >= 0)
                    {
                        if (!hasReadPostData)
                        {
                            ...
                            parseRequest(req_str, "Content-Length:",
                                    content_length);
...
// Serial.println(String("read temperature i=") + i + " j=" + j);
        ...
        {
            Serial.println(
                    String("ERROR: bad temperature: ") + i + " - " + temp);
            temp = 0;
        }
        if (temp)
            temperature[i] = temp;
...
// Serial.println("Start checkProgramming");
    ...
    for (byte i = 0; i < SENZOR_COUNT; ++i)
    {
        ...
        case P1_RUN_HOURS_MAKE_TEMP: // keep target temperature during time interval
        {
            ...
            should_run[i] = temperature[i] <= target_temp
                    && LsuTimeHelper::isNowBetween(start_hour_p1[i],
                            start_minute_p1[i], stop_hour_p1[i],
                            stop_minute_p1[i]);
            break;
        }
        case P2_START_HOUR_1_INCREASE_05: // start at given time and increase temperature with DELTA_TEMP
        {
            should_run[i] = !has_p2_run_today[i]
                    && LsuTimeHelper::isNowAfter(start_hour_p2[i],
                            start_minute_p2[i]);
            if (should_run[i] && !target_temperature_p2[i])
            {
                ...
                target_temperature_post += String("&") + TARGET_TEMPERATURE_P2
                        + "_" + i + "="
        ...
        case P3_START_HOUR_2_INCREASE_05: // start at given hour and increase temperature with DELTA_TEMP
        {
            should_run[i] = !has_p3_run_today[i]
                    && LsuTimeHelper::isNowAfter(start_hour_p3[i],
                            start_minute_p3[i]);
            if (should_run[i] && !target_temperature_p3[i])
            {
                target_temperature_p3[i] = temperature[i] + DELTA_TEMP;
                send_target_temperature_update = true;
                target_temperature_post += String("&") + TARGET_TEMPERATURE_P3
                        + "_" + i + "="
        ...
        case P4_START_NOW_INCREASE_05: // start now and increase temperature with DELTA_TEMP
        {
            if (!target_temperature_p4[i])
            {
                ...
                send_target_temperature_update = true;
                target_temperature_post += String("&") + TARGET_TEMPERATURE_P4
                        + "_" + i + "="
    ...
    if (run_marked)
        writeLogger(
                String("[") + T_LOC
                        + "]  P2 si P3 marcate ca nu au rulat astazi");
    ...
    if (one_delta_running)
    {
        ...
        for (int i = 0; i < SENZOR_COUNT; ++i)
        {
            if (should_run[i] || programming[i] != P1_RUN_HOURS_MAKE_TEMP)
                continue;
            int candidate_temp = temperature[i]
                    - target_temperature_p1[i + offset];
    ...
    else
    {
        ...
        for (int i = 0; i < SENZOR_COUNT; ++i)
        {
            ...
            {
                candidates_temp[i] = temperature[i]
                        - target_temperature_p1[i + offset];
                // add a bonus to already running to prevent bouncing
                if (is_running[i])
        ...
        if (should_run[i] && !is_running[i])
        {
            ...
            writeLogger(
                    String("[") + T_LOC + "] " + room_name[j]
                            + " - start program " + printProgramming(i)
                            + "; temperatura curenta " + temp);
            delay(25);
        }
    }
    for (int i = 0, j = offset; i < SENZOR_COUNT; ++i, ++j)
    {
        if (force_running[i] && !is_running[i])
        {
            ...
            writeLogger(
                    String("[") + T_LOC + "] " + room_name[j]
                            + " - start fortat program " + printProgramming(i)
                            + "; temperatura curenta " + temp);
            delay(25);
        }
    }
    for (int i = 0, j = offset; i < SENZOR_COUNT; ++i, ++j)
    {
        if (!should_run[i] && !force_running[i] && is_running[i])
        {
            ...
            writeLogger(
                    String("[") + T_LOC + "] " + room_name[j]
                            + " - stop program " + original_programming[i]
                            + "; temperatura curenta " + temp);
            delay(25);
        }
    }
