...
{ .write = send_to_peer, .read = read_from_peer, .event = NULL,
#ifdef DTLS_PSK
...
int main(int argc, char **argv)
{
  ...
  /* use port number from command line when specified or the listen
   port, otherwise */
  dst.addr.sin.sin_port = htons(
      atoi(optind < argc ? argv[optind++] : port_str));
    ...
    if (len)
    {
              ...
              strlen(DTLS_CLIENT_CMD_RENEGOTIATE)))
      {
        ...
        len = 0;
          ...
          && !memcmp(buf, DTLS_CLIENT_CMD_REHANDSHAKE,
              strlen(DTLS_CLIENT_CMD_REHANDSHAKE)))
      {
